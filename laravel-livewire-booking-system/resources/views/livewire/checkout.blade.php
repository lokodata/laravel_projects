<form class="space-y-10" wire:submit="submit">
    <div>
        <h2 class="text-xl text-medium">Here's what you're booking</h2>

        <div class="mt-6 mb-6 flex space-x-3 bg-slate-100 rounded-lg p-4">

            @if ($employee)
                <img src="{{ $employee->profile_photo_url }}" alt="" class="rounded-lg size-14 shrink-0">
            @else
        ...
        <div>
            ...
            <input x-on:select="$wire.setDate($event.detail)" x-data
                x-picker="{
                    date: '{{ $form->date }}',
                    availability: {{ $this->availabilityJson }},
                }"
                class="mt-6 mb-6 text-sm bg-slate-100 border-0 rounded-lg px-6 py-4 w-full" placeholder="Choose a date">
                ...
                <div class="grid grid-cols-3 md:grid-cols-5 gap-8">
                    ...
                    @if ($this->times->isNotEmpty())
                        @foreach ($this->times as $time)
                            <button wire:click="setTime('{{ $time }}')" type="button"
                                @class([
        ...
        @if ($form->time)
            <div>
                ...
                @error('form.time')
                    <div class="bg-slate-900 text-white py-4 px-6 rounded-lg mt-6">
                        {{ $message }}
                    </div>
                @enderror
